...
local Config = Super:New ( {
    ...
    BuildControls = function (self)
        ...
        local idevices = c:GetInterfaceDevices ()

        local list = {}
        
        for _,idevice in pairs (idevices) do
            table.insert (list,idevice:GetDisplayName ())
        end
...
print ('fn',self.Controls.image.Filename)
        Super.OnShow (self)
    end,
