...
local asynctimeout = 30000 -- wait 30 seconds for an asyncexecute call to timeout
...
--        print ('upnp xpl event handler',event,unpack (arg))

        if event == 'xPLDeviceLeft' and arg [1] =='tieske-upnp.media' then -- gateway left
            print '*** gate way left'
            for k,v in pairs(self.DevList) do
                self:DeleteRootDevice(v)
            end
	...
	CallMethodAsync = function (self, method, callback, ...)
        ...
        queueItem= {
            ...
            TimeOut = date:now(),
            CallBack = callback,
            DED = Classes.DelayedExecutionDispatcher:New (asynctimeout, 
                function ()
                    local result, err = pcall(queueItem.CallBack, false, 'No response from device')
                    if not result then
                        print ("Error returned by the callback for a UPnP method call result;")
                        print ("    CallID  : " , queueItem.CallID)
                        print ("    MethodID: " , queueItem.MethodID)
                        print ("    Error   : " , err)
                    end
                end
            ),
    ...
    HandleMethodResult = function(self, msg)
        ...
        if callid ~= nil then
            if self.ResponseQueue[callid] ~= nil then
                ...
                local myCall = self.ResponseQueue[callid]
                _ = myCall.DED and myCall.DED:Close ()
                self.ResponseQueue[callid] = nil
                -- deal with reponse
                local response = {}
